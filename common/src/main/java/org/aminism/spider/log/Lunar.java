...
/**
 * ========================================================= * * 作者:dzl * * 版本:1.0 * * 创建日期:2015-7-4 下午3:24:55 * * 描述:公历转农历工具 * * 修订历史: * * ==========================================================
 */
public class Lunar {
    /**
    ...
    public String getLunarDate() {
        String s = threadLocal.get().format(calendar.getTime()) + " ";
    ...
    public String getLunarHoliday() {
        int temp;
        if (result[2] == 29 &&
                !isBigMonth(result[0], result[1])) {//如果是小月,29日为最后一天,将其加为30日算
            temp = (result[1] << 8) + 30;
        } else {
    ...
    public Lunar build() {
        ...
        this.lunaryear = cyclical(result[4]);
        this.ganzhiMonth = cyclical(result[5]) + "月";
        this.ganzhiDay = cyclical(result[6]) + "日";
        SimpleDateFormat sdf = new SimpleDateFormat("EEEEE");
        this.week = sdf.format(calendar.getTime());
        this.animal = Animals[(result[0] - 4) % 12];
        return this;
    }
